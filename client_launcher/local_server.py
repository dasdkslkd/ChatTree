...
def _configured_binary_argv(value: str | os.PathLike[str]) -> list[str]:
    raw = os.fspath(value)
    parts = shlex.split(raw, posix=os.name != "nt")
    if not parts:
        raise ValueError("server_binary must not be empty")
    normalized = [part.strip("\"'") for part in parts]
    executable = os.path.expanduser(normalized[0])
    if os.path.dirname(executable):
        executable = str(Path(os.path.abspath(executable)))
...
class LocalServerError(LauncherError):
    def __init__(
        ...
        *,
        ...
        super().__init__(code, message, retryable, status_code, details=details)
        self.phase = phase
...
class LocalServerConnector:
    ...
    def _validate_handshake(self, payload: Mapping[str, Any]) -> str:
        ...
        version = str(payload["server_version"])
        check = check_server_version(version)
        if not check.compatible:
            raise LocalServerProtocolError(
                "local_server_version_mismatch",
                (
                    "Local server version is incompatible: "
    ...
    def _spawn(
        ...
        argv = self._server_serve_argv(port, server_home)
        env = os.environ.copy()
        env.update(
            {
    ...
    def _server_serve_argv(self, port: int, server_home: Path) -> list[str]:
        binary = getattr(self._settings, "server_binary", None)
        if binary:
            return [
                *_configured_binary_argv(binary),
                "serve",
                "--host",
                "127.0.0.1",
                "--port",
