...
- For known paths, use one batched `read` without a preceding `glob`. After reading the relevant files, emit independent edits together, then run one consolidated validation; do not add `read` or `grep` calls that cannot change the next action.
